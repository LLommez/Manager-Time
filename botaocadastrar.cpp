...
void save_professor(Fl_Input* name_input, Fl_Input* subjects_input) {
    std::ofstream file("professores.txt", std::ios_base::app);
    if (file.is_open()) {
...
void show_professors(Fl_Multiline_Output* output) {
    ...
    if (file.is_open()) {
        while (std::getline(file, line)) {
            ...
            display_text += "Professor: " + parts[0] + "\nMatérias: " + parts[1] + "\n\n";
        }
        file.close();
    }
...
void delete_professor(const std::string& name) {
    std::ifstream file("professores.txt");
    ...
    if (file.is_open() && temp.is_open()) {
        ...
        file.close();
        temp.close();
        std::remove("professores.txt");
        std::rename("temp.txt", "professores.txt");
    }
}
...
int main() {
    ...
    Fl_Window* window = new Fl_Window(window_width, window_height, "Cadastro de Professores");
...
// Verifica se a imagem foi carregada corretamente
    ...
    Fl_Input* subjects_input = new Fl_Input(0, 70, 200, 25, "Matérias:");

    Fl_Button* save_button = new Fl_Button(0, 110, 200, 25, "Cadastrar Professor");
    Fl_Button* show_button = new Fl_Button(0, 150, 200, 25, "Professores Cadastrados");

    // Campo e botão para excluir professor
    Fl_Input* delete_input = new Fl_Input(0, 190, 200, 25, "Excluir Nome:");
    Fl_Button* delete_button = new Fl_Button(0, 230, 200, 25, "Excluir Professor");
